...
const ViewLive = ({ open, handleClose, id }: ViewLiveProps) => {
  const dispatch = useAppDispatch();

  const dimensions = useWindowDimensions();

  const [openMessageD, setOpenMessageD] = useState(false);
  const [openTourD, setOpenTourD] = useState(false);
  const [openProfileD, setOpenProfileD] = useState(false);

  const handleToggleTourD = () => setOpenTourD((prev) => !prev);
  ...
  return (
    <Modal visible={open} onDismiss={handleClose} style={{ padding: 20 }}>
      ...
      <View className="relative pb-24">
        ...
        <View className="p-2 absolute bottom-5 flex-row justify-center gap-x-4  w-full border-gray-light bg-white  border-t">
          <Button mode="outlined" onPress={handleToggleMessageD}>
            Send message
          </Button>
          <Button mode="contained" onPress={handleToggleTourD}>
            Request a tour
          </Button>
        </View>
      </View>
