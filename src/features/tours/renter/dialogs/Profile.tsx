...
const Profile = ({ open, handleClose, tour }: ProfileProps) => {
  ...
  const onSubmit = async (data: Inputs) => {
    ...
    formData.append("recipient", tour.agent._id!);
        ...
        <Dialog.Content className="rounded-xl  p-0 ">
          <View className="flex-row justify-between text-right items-center  p-4">
            <Text className="text-lg">
              Get in touch with {tour.agent?.username.split(" ").shift()}{" "}
            </Text>
              ...
              {[...Array(2)].map((_, i) => {
                ...
                return (
                  <IconButton
                    ...
                    onPress={() =>
                      i === 0
                        ? Linking.openURL(`tel:${tour.agent?.phoneNumber}`)
                        : Linking.openURL(
                            `whatsapp://send?phone=${tour.agent?.phoneNumber}`
                          )
