...
import { useGetRenterToursQuery } from "./tourApiSlice";
...
type RenterToursProps = {
  notifications: INotificationResult;
};

const RenterTours = ({ notifications }: RenterToursProps) => {
  ...
  /* -------------------------------------------------------------
   ...
   ----------------------------------------------------------------*/
  const {
    currentData: data, //currentData////The latest returned result for the current hook arg, if present.
    //data////The latest returned result regardless of hook arg, if present.
    isFetching, //query is currently fetching, but might have data from an earlier request. This will be true for both the first request fired off, as well as subsequent requests.
    isSuccess,
    isError,
    error,
    refetch,
    isLoading, //query is currently loading for the first time, and has no data yet.
  ...
  useEffect(() => {
    if (!status) return;
    //as a renter, notifications are only Upcoming when req is confirmed
    //will only show this notification
    if (status === "Upcoming") {
    ...
    <View className="flex-1 p-3">
            ...
            buttons={STATUS.map((value) => ({
              ...
              icon: ({ size, color }) => (
                <Badge
                  size={20}
                  visible={
                    value === TourStatus.Upcoming
                      ? !!notification?.upcoming
                      : false
                  }
                  style={{}}
                >
                  {value === TourStatus.Upcoming ? notification?.upcoming : 0}
                </Badge>
              ),
            }))}
          />
        ...
        <View className="mt-3">
          <Text className="text-gray-muted">
            ...
            {status === TourStatus.Unconfirmed &&
              "It is almost done. Your tour requests below will be confirmed shortly"}
            {status === TourStatus.Rescheduled &&
              "Your rescheduled tours will be confirmed shortly"}
            {status === TourStatus.Completed &&
              "Congrats! Hope you got a good place to rent."}
          </Text>
        </View>
      )}
      ...
      {!toursData.length && !isFetching && (
        <View className="gap-y-4 py-10 items-center">
          ...
          <Text>
            When you request tours for vacant houses, the requests will appear
            here.
          </Text>
          ...
          <View className="flex-row gap-x-3">
            <Button compact mode="outlined" onPress={handleResetFilters}>
              Reset filters
            </Button>
            <Button compact mode="outlined" onPress={handleRoleSwitch}>
