...
import { useCancelTourMutation, useDeclineTourMutation } from "../tourApiSlice";
import { Controller, useForm } from "react-hook-form";

type DeclineTourProps = {
  open: boolean;
  handleClose: () => void;
  tour: ITour;
};

const DeclineTour = ({ open, handleClose, tour }: DeclineTourProps) => {
  const [declineTour, { isLoading, isSuccess, isError, error, data }] =
  ...
  const onSubmit = async (data: Inputs) => {
    await declineTour({ ...data, id: tour._id });
  };
      ...
      >
        ...
        <Dialog.Content className="rounded-xl  p-0 ">
          <View className="flex-row justify-between text-right items-center  p-4">
            <Text className="text-lg"> Decline tour</Text>
            <EvilIcons name="close" size={24} onPress={handleClose} />
          </View>

          <View className="p-5 pb-8">
            <Text className="mb-3">
              We don't recommend declining tour requests unless it is really
              necessary.
            </Text>

            <Text className="mb-3">
              If you aren't available on the dates suggested, you could send the
              client a message asking them to reschedule to a later date.
            </Text>

            <Text className="mb-3">Enter reason for declining the tour</Text>
